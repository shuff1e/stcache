...
func newRaftTransport(opts *options,w io.Writer) (*raft.NetworkTransport, error) {
	address, err := net.ResolveTCPAddr("tcp", opts.raftTCPAddress)
	if err != nil {
		return nil, err
	}
	transport, err := raft.NewTCPTransport(address.String(), address, 3, 10*time.Second, w)
	if err != nil {
		return nil, err
	}
...
func newRaftNode(opts *options, ctx *stCachedContext, gcm *gossipCacheManager,
	logOutput *lumberjack.Logger) (*raftNodeInfo, error) {
	raftConfig := raft.DefaultConfig()
	raftConfig.LocalID = raft.ServerID(opts.raftTCPAddress)
	//raftConfig.Logger = log.New(os.Stderr, "raft: ", log.Ldate|log.Ltime)
	raftConfig.Logger = log.New(logOutput,"HA service raft: ",log.Ldate|log.Ltime)
	raftConfig.SnapshotInterval = 20 * time.Second
	raftConfig.SnapshotThreshold = 2
	leaderNotifyCh := make(chan bool, 1)
	raftConfig.NotifyCh = leaderNotifyCh

	transport, err := newRaftTransport(opts,logOutput)
	if err != nil {
		return nil, err
	}
	...
	fsm := &FSM{
		ctx: ctx,
		log: log.New(logOutput, "FSM: ", log.Ldate|log.Ltime),
		gcm: gcm,
	}
	snapshotStore, err := raft.NewFileSnapshotStore(opts.dataDir, 1, logOutput)
	if err != nil {
		return nil, err
	}
